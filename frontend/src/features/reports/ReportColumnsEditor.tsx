...
export function ReportColumnsEditor({
  ...
  return (
    <section>
      <h2 className="font-bold">Columnas</h2>
      <p className="mt-1 text-sm text-slate-500">
        Configura visibilidad, orden, etiqueta y formato.
      </p>
      <div className="mt-3 space-y-2">
        {columns.map((column, index) => (
          <div
            className="grid gap-2 rounded-lg border p-3 md:grid-cols-[auto_1fr_150px_120px_auto] md:items-center"
            key={column.source_key}
          >
            <input
            ...
            />
            <input
              aria-label={`Etiqueta de ${column.source_key}`}
              className="field"
              value={column.label}
              onChange={(event) => {
            ...
            />
            <select
              aria-label={`Formato de ${column.source_key}`}
              className="field"
              value={column.format.type}
              onChange={(event) => {
                update(index, { format: { ...column.format, type: event.target.value } });
              }}
            >
              {[
                "automatic",
            ...
            <select
              aria-label={`Alineación de ${column.source_key}`}
              className="field"
              value={column.alignment}
              onChange={(event) => {
                update(index, { alignment: event.target.value as ReportColumn["alignment"] });
              }}
            >

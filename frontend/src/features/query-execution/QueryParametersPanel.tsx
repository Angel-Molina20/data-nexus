      ...
      {parameters.map((parameter) => (
        <label className="text-xs font-semibold text-slate-700" key={parameter.parameter_id}>
          ...
          <ParameterInput
            ...
            onChange={(value) => {
              onChange(parameter.parameter_id, value);
            }}
          />
          <span className="block font-normal text-slate-500">{parameter.description}</span>
        </label>
      ))}
    </fieldset>
  );
...
function ParameterInput({
  ...
  if (parameter.allowed_values?.length)
    return (
      ...
      >
        ...
        {parameter.allowed_values.map((item) => (
  ...
  const inputType =
    parameter.data_type === "date"
      ...
      : parameter.data_type === "datetime"
        ? "datetime-local"
        : ["integer", "decimal", "float"].includes(parameter.data_type)
          ? "number"
          : parameter.sensitive
            ? "password"
            : "text";
  return (
    <input
      ...
      autoComplete={parameter.sensitive ? "off" : undefined}
    />
  );
}

...
import { getSchemaEntity } from "./api/schemaApi";
    ...
    <section className="min-w-0 rounded-xl border border-slate-200 bg-white">
      <div className="border-b border-slate-200 p-5">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {entity.entity_type} · {entity.schema_name}
        </p>
        <div className="mt-3 flex flex-wrap gap-2">
          <Link className="btn-secondary" to={`/connections/${connectionId}/relationships/new`}>
            Crear relación
          </Link>
          <Link className="btn-secondary" to={`/connections/${connectionId}/semantic-catalog`}>
            Editar metadatos semánticos
          </Link>
        </div>
      </div>
              ...
              <tbody>
                {entity.fields.map((field) => (
                  <tr className={!field.is_active ? "opacity-50" : ""} key={field.id}>
                    ...
                    <td>{field.normalized_data_type}</td>
                    <td>{field.is_nullable ? "Sí" : "No"}</td>
                    <td>
                      <span className="block max-w-40 truncate">
                        {formatDefaultValue(field.default_value)}
                      </span>
                    </td>
                    <td>
                      <div className="flex gap-1">
                        {field.is_primary_key ? <StatusBadge variant="info">PK</StatusBadge> : null}
                        {field.is_unique ? <StatusBadge>Unique</StatusBadge> : null}
                        {field.is_auto_increment ? <StatusBadge>Auto</StatusBadge> : null}
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {entity.indexes.map((index) => (
              <article className="rounded-lg border border-slate-200 p-4" key={index.id}>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <strong>{index.physical_name}</strong>
                  {index.is_primary ? <StatusBadge variant="info">Primary</StatusBadge> : null}
                  {index.is_unique ? <StatusBadge>Unique</StatusBadge> : null}
                </div>
                <p className="mt-2 text-sm text-slate-500">
                  {index.fields
                    .map((field) => `${String(field.sequence)}. ${field.field_name ?? "expresión"}`)
                    .join(" · ")}
                </p>
              </article>
            ))}
              ...
              <article className="rounded-lg border border-slate-200 p-4" key={relationship.id}>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  {relationship.fields
                    ...
                    .join(", ")}{" "}
                  · UPDATE {relationship.update_rule} · DELETE {relationship.delete_rule}
                </p>
              </article>
            ))}
            {!relationships.length ? (
              <p className="text-sm text-slate-500">No existen claves foráneas físicas.</p>
            ) : null}
            <Link
              className="rounded-lg bg-blue-50 p-3 text-sm font-semibold text-blue-800"
              to={`/connections/${connectionId}/relationships`}
            >
              Ver relaciones físicas, inferidas, manuales y polimórficas
            </Link>
          </div>
        ) : null}
        {tab === "information" ? (
          <dl className="detail-grid">
            ...
            <div>
              <dt>Motor</dt>
              <dd>{entity.storage_engine ?? "No informado"}</dd>
            </div>
            <div>
              <dt>Collation</dt>
              <dd>{entity.collation ?? "No informada"}</dd>
            </div>
            <div>
              <dt>Filas estimadas</dt>
              <dd>{entity.estimated_rows ?? "No disponible"}</dd>
            </div>
            <div>
              <dt>Primera detección</dt>
              <dd>{new Date(entity.first_seen_at).toLocaleString()}</dd>
            </div>
            <div>
              <dt>Última detección</dt>
              <dd>{new Date(entity.last_seen_at).toLocaleString()}</dd>
            </div>
            <div className="sm:col-span-2">
              <dt>Comentario</dt>
              <dd>{entity.comment || "Sin comentario"}</dd>
            </div>
            <p className="text-xs text-slate-500 sm:col-span-2">
              El número de filas es una estimación del motor, no un conteo exacto.
            </p>
          </dl>
        ) : null}
      </div>

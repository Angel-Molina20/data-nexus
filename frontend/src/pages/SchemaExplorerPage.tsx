...
import {
  ...
  synchronizeSchema,
} from "../features/schema/api/schemaApi";
...
export function SchemaExplorerPage() {
  ...
  const summary = useQuery({
    queryKey: ["schema-summary", id],
    queryFn: () => getSchemaSummary(id),
  });
  const entities = useQuery({
    queryKey: ["schema-entities", id, search, entityType, showInactive],
    queryFn: () =>
      listSchemaEntities(id, {
        search,
        entityType,
  ...
  if (summary.isPending)
    return (
      <PageContainer>
        <p className="state-message">Cargando esquema…</p>
      </PageContainer>
    );
  if (summary.isError)
    return (
      <PageContainer>
        <p className="alert-error">No fue posible cargar el resumen del esquema.</p>
      </PageContainer>
    );
    ...
    <PageContainer>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: metadata.connection_name, to: routes.connections.detail(id) },
          { label: "Esquema" },
        ]}
        actions={
          <>
            <Link className="btn-secondary" to={`/connections/${id}/schema/synchronizations`}>
      ...
      {synchronize.isSuccess ? (
        <p className="alert-success">Sincronización completada correctamente.</p>
      ) : null}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {[
          ...
          ["Campos", metadata.fields],
          ["Índices", metadata.indexes],
      ...
      {!metadata.status ? (
        <div className="rounded-xl border border-slate-200 bg-white">
          <EmptyState
            icon={Database}
            title="Esquema aún no sincronizado"
            description="Pulsa Sincronizar esquema para leer exclusivamente metadatos estructurales."
          />
        </div>
      ) : (
        <div className="grid min-w-0 gap-4 xl:grid-cols-[310px_minmax(0,1fr)]">
          <aside className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-4 grid max-h-[620px] gap-1 overflow-y-auto">
              {entities.isPending ? (
                <p className="p-3 text-sm text-slate-500">Cargando entidades…</p>
              ) : null}
              {entities.data?.items.map((entity) => (
                <Link
                  className={`rounded-lg p-3 transition ${entity.id === entityId ? "bg-blue-50 text-blue-800" : "hover:bg-slate-50"}`}
                  key={entity.id}
                  to={`/connections/${id}/schema/entities/${entity.id}`}
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="flex min-w-0 items-center gap-2">
                      <Table2 className="size-4 shrink-0" />
                      <strong className="truncate text-sm">{entity.physical_name}</strong>
                    </span>
                    <StatusBadge>{entity.entity_type}</StatusBadge>
                  </div>
                  <p className="mt-1 text-xs text-slate-500">
                    {entity.fields_count} campos · {entity.indexes_count} índices ·{" "}
                    {entity.relationships_count} relaciones
                  </p>
                </Link>
              ))}
          ...
          {entityId ? (
            <EntityDetailPanel connectionId={id} entityId={entityId} />
          ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-3 grid gap-2">
            {changes.data?.items.slice(0, 6).map((change) => (
              <p className="text-sm text-slate-600" key={change.id}>
                <strong aria-label={change.change_type}>
                ...
                {change.object_type} {change.physical_name}
              </p>
            ))}
            {changes.data?.items.length === 0 ? (
              <p className="text-sm text-slate-500">Sin cambios recientes.</p>
            ) : null}
          </div>
        </section>
        <section className="rounded-xl border border-slate-200 bg-white p-5">

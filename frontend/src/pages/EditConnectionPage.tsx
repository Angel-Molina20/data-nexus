...
import { getConnection, updateConnection } from "../services/connections";
...
export function EditConnectionPage() {
  ...
  useEffect(() => {
    if (query.data && !initialized.current) {
      form.reset({ name: query.data.name, engine: "mysql", host: query.data.host, port: query.data.port, database_name: query.data.database_name, username: query.data.username, password: "", ssl_enabled: query.data.ssl_enabled, configuration: query.data.configuration });
      initialized.current = true;
    }
  }, [form, query.data]);
  const technical = form.formState.dirtyFields.host || form.formState.dirtyFields.port || form.formState.dirtyFields.database_name || form.formState.dirtyFields.username || form.formState.dirtyFields.password || form.formState.dirtyFields.ssl_enabled;
  const update = useMutation({
    ...
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["connections"] }); void navigate(`/connections/${id}`, { state: { message: "Conexión actualizada." } }); },
  });
      ...
      <PageSection>
        <form className="grid gap-7" onSubmit={(event) => { void form.handleSubmit((data) => { update.mutate(data); })(event); }}>
          ...
          <div className="flex flex-wrap justify-end gap-3">
            <button className="btn-primary" disabled={update.isPending} type="submit">{update.isPending ? "Probando y actualizando…" : technical ? "Probar y guardar cambios" : "Guardar cambios"}</button>
          </div>
        </form>
      </PageSection>
    </PageContainer>

...
import { listSchemaSynchronizations } from "../features/schema/api/schemaApi";
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        actions={
          <Link className="btn-secondary" to={`/connections/${id}/schema`}>
            <ArrowLeft className="size-4" /> Volver al esquema
          </Link>
        }
      />
      <PageSection>
            ...
            <tbody>
              {query.data?.items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{item.entities_discovered}</td>
                  <td>{item.fields_discovered}</td>
                  <td>{item.indexes_discovered}</td>
                  <td>{item.relationships_discovered}</td>
                  <td>
                    +{item.entities_added + item.fields_added} ~
                    {item.entities_updated + item.fields_updated} −
                    {item.entities_removed + item.fields_removed}
                  </td>
                </tr>
              ))}
            </tbody>

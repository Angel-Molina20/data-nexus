  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        actions={<><button className="btn-secondary" disabled={retest.isPending} onClick={() => { retest.mutate(); }}><RefreshCw className="size-4" />{retest.isPending ? "Probando…" : "Probar"}</button><Link className="btn-secondary" to={`/connections/${id}/edit`}><Pencil className="size-4" /> Editar</Link><button className="btn-danger" disabled={remove.isPending} onClick={() => { if (window.confirm("¿Eliminar esta configuración local?")) remove.mutate(); }}><Trash2 className="size-4" /> Eliminar</button></>}
      />

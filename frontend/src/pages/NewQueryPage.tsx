...
export function NewQueryPage() {
  ...
  const connections = useQuery({
    queryKey: ["connections", "query-picker"],
    queryFn: () => listConnections({}),
  });
  const entities = useQuery({
    queryKey: ["schema-entities", connectionId],
    ...
    <PageContainer>
      ...
      >
        ...
        <label className="grid gap-2 text-sm font-semibold">
          ...
          >
            <option value="empty">Consulta vacía</option>
            <option value="fields">Seleccionar campos de una entidad</option>
            <option value="filter">Consulta con filtro</option>
            <option value="aggregate">Consulta agregada</option>
            <option value="relationship">Consulta con relación</option>
          </select>
        </label>
        <p className="text-sm text-slate-500">
          La plantilla crea únicamente nodos universales y puede completarse en el editor JSON.
        </p>
        {create.isError ? (
...
function initialDocument(connectionId: string, entityId: string, template: string): QueryDocument {
  ...
  return {
      ...
      select: [
        {
          ...
          expression: literal,
          alias: "value",
          label: template === "empty" ? "Valor inicial" : `Plantilla ${template}`,
          hidden: false,
        },
      ],
      group_by: [],
    ...
    metadata: {
      ...
      notes: null,
      builder_layout: {
        nodes: { src_main: { x: 100, y: 80, collapsed: false } },

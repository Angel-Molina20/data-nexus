...
export function PolymorphicRelationshipPage() {
  ...
  const create = useMutation({
    mutationFn: () =>
      createPolymorphicRelationship(id, {
    ...
    () =>
      Boolean(
        sourceEntity &&
          ...
          name.trim() &&
          mappings.length &&
          mappings.every((item) => item.typeValue && item.targetEntity && item.targetField),
      ),
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <label className="grid gap-2 text-sm font-semibold">
            ...
            >
              <option value="">Seleccionar…</option>
              {source.data?.fields
                .filter((field) => ["string", "text", "enum"].includes(field.normalized_data_type))
                .map((field) => (
        ...
        <div className="mt-6 grid gap-3">
          ...
          {mappings.map((mapping, index) => (
            <div
              className="grid gap-3 rounded-lg bg-slate-50 p-4 lg:grid-cols-4"
              key={String(index)}
            >
              <label className="grid gap-1 text-xs font-semibold">
                ...
                <input
                  className="field"
                  placeholder="Student"
                  value={mapping.typeValue}
                  onChange={(event) => {
                    updateMapping(index, { typeValue: event.target.value });
                  }}
                />
              </label>
              <label className="grid gap-1 text-xs font-semibold">
                ...
                <select
                  ...
                  onChange={(event) => {
                    updateMapping(index, { targetEntity: event.target.value, targetField: "" });
                  }}
                >
                  <option value="">Seleccionar…</option>
                  {entities.data?.items.map((item) => (
              ...
              <label className="grid gap-1 text-xs font-semibold">
                ...
                <select
                  ...
                  onChange={(event) => {
                    updateMapping(index, { targetField: event.target.value });
                  }}
                >
                  <option value="">Seleccionar…</option>
                  {targets.data?.[index]?.fields
              ...
              {source.data && mapping.typeValue && targets.data?.[index] ? (
                <p className="alert-success lg:col-span-4">
                  {source.data.physical_name}.
                  {source.data.fields.find((field) => field.id === typeField)?.physical_name} =
                  &quot;{mapping.typeValue}&quot; AND {source.data.physical_name}.
                  {source.data.fields.find((field) => field.id === idField)?.physical_name} ={" "}
                  {targets.data[index].physical_name}.
                  {
                    targets.data[index].fields.find((field) => field.id === mapping.targetField)
                      ?.physical_name
                  }
                </p>
              ) : null}
        ...
        <p className="alert-warning mt-5">
          Nunca se relacionará únicamente el campo identificador: el discriminador y el ID forman
          una condición conjunta obligatoria.
        </p>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          >
            {create.isPending ? "Guardando…" : "Crear relación polimórfica"}
          </button>
        </div>
      </section>

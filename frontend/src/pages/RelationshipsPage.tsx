...
export function RelationshipsPage() {
  ...
  if (relationships.isPending) {
    return (
      <PageContainer>
        <p className="state-message">Cargando relaciones…</p>
      </PageContainer>
    );
  }
  if (relationships.isError) {
    return (
      <PageContainer>
        <p className="alert-error">No fue posible cargar las relaciones.</p>
      </PageContainer>
    );
  }
    ...
    <PageContainer>
      <PageHeader
        ...
        actions={
          <>
            ...
            <Link className="btn-secondary" to={`/connections/${id}/relationships/polymorphic/new`}>
              <GitFork className="size-4" /> Polimórfica
            </Link>
            <Link className="btn-primary" to={`/connections/${id}/relationships/new`}>
              <Plus className="size-4" /> Relación manual
            </Link>
          </>
        }
      />
      {detect.isSuccess ? (
        <p className="alert-success">
          Detección completada: {detect.data.created} sugerencia(s) nueva(s).
        </p>
      ) : null}
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-3">
        ...
        >
          ...
          <option value="inferred">Inferida</option>
          <option value="manual">Manual</option>
          <option value="polymorphic">Polimórfica</option>
        </select>
        <select
      ...
      {view === "graph" ? (
        graph.isPending ? (
          <p className="state-message">Preparando grafo…</p>
        ) : graph.data ? (
          <RelationshipGraph graph={graph.data} />
        ) : (
          <p className="alert-error">No se pudo cargar el grafo.</p>
        )
      ) : (
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          {filtered.map((item) => (
            <article
              className="grid gap-3 border-b border-slate-100 p-5 lg:grid-cols-[1fr_auto] lg:items-center"
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  ...
                  {item.target
                    ...
                    : "destinos según mapping"}
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  {item.cardinality} · Confianza {Math.round(item.confidence * 100)}% ·{" "}
                  {item.detection_source}
                </p>
                {item.invalid_reason ? (
                  <p className="mt-2 text-sm text-red-700">{item.invalid_reason}</p>
                ) : null}
              </div>
              {item.status === "suggested" ? (
                <Link className="btn-secondary" to={`/connections/${id}/relationships/candidates`}>
                  <Sparkles className="size-4" /> Revisar sugerencia
                </Link>
              ) : null}
            </article>
          ))}
          {!filtered.length ? (
            <p className="state-message">No hay relaciones para estos filtros.</p>
          ) : null}
        </section>
      )}
      {summary.bridge_candidates.map((item) => (
        <p className="alert-warning" key={item.entity_id}>
          {item.message} Entidad: {item.entity_name} ({item.reference_fields.join(", ")}).
        </p>
      ))}
    </PageContainer>
  );

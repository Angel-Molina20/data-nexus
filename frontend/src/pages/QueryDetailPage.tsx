...
export function QueryDetailPage() {
  ...
  return <PageContainer>
    <PageHeader eyebrow={`AST ${item.schema_version}`} title={item.name} description={item.description || "Borrador universal sin descripción."} actions={<div className="flex gap-2"><Link className="btn-primary" to={`/queries/${item.id}/builder`}><Network className="size-4" />Constructor</Link>{hasPermission("queries.compile") && item.validation_status === "valid" ? <Link className="btn-secondary" to={`/queries/${item.id}/compile`}><Code2 className="size-4" />Compilar</Link> : null}{hasPermission("queries.update") ? <Link className="btn-secondary" to={`/queries/${item.id}/edit-json`}><FileJson2 className="size-4" />Editar JSON</Link> : null}</div>} />

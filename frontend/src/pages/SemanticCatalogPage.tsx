...
export function SemanticCatalogPage() {
  ...
  const saveField = useMutation({
    mutationFn: ({ fieldId, payload }: { fieldId: string; payload: Record<string, unknown> }) => updateSemanticField(id, fieldId, payload),
    onSuccess: async () => {
...
function SemanticEditor({ entity, saving, onSaveEntity, onSaveField }: {
  ...
  useEffect(() => { setDisplayName(entity.display_name); setDescription(entity.description ?? ""); setDomain(entity.business_domain ?? ""); }, [entity]);
  return <section className="min-w-0 rounded-xl border border-slate-200 bg-white p-5"><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-semibold">{entity.display_name}</h2><StatusBadge>{entity.is_active ? "Activa" : "Entidad física inactiva"}</StatusBadge></div><p className="mt-1 text-sm text-slate-500">Nombre físico: <code>{entity.physical_name}</code></p><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm font-semibold">Nombre de negocio<input className="field" value={displayName} onChange={(event) => { setDisplayName(event.target.value); }} /></label><label className="grid gap-2 text-sm font-semibold">Dominio<input className="field" value={domain} onChange={(event) => { setDomain(event.target.value); }} /></label><label className="grid gap-2 text-sm font-semibold md:col-span-2">Descripción<textarea className="field min-h-20" value={description} onChange={(event) => { setDescription(event.target.value); }} /></label></div><button className="btn-primary mt-4" disabled={saving} onClick={() => { onSaveEntity({ display_name: displayName, description: description || null, business_domain: domain || null }); }}>Guardar entidad</button><h3 className="mt-8 font-semibold">Campos</h3><div className="mt-3 overflow-x-auto"><table className="data-table"><thead><tr><th>Físico</th><th>Nombre visible</th><th>Tipo semántico</th><th>Visible</th><th>Sensible</th><th /></tr></thead><tbody>{entity.fields.map((field) => <SemanticFieldRow field={field} key={field.id} saving={saving} onSave={onSaveField} />)}</tbody></table></div></section>;
}

function SemanticFieldRow({ field, saving, onSave }: {
  ...
  return <tr><td><strong>{field.physical_name}</strong></td><td><input className="field min-w-36" value={name} onChange={(event) => { setName(event.target.value); }} /></td><td><select className="field min-w-40" value={semanticType} onChange={(event) => { setSemanticType(event.target.value); }}>{["unknown","identifier","foreign_identifier","person_name","email","phone","date","datetime","currency","percentage","status","category","description","code","url","boolean"].map((item) => <option key={item} value={item}>{item}</option>)}</select></td><td><input aria-label={`Visibilidad de ${field.physical_name}`} checked={visible} type="checkbox" onChange={(event) => { setVisible(event.target.checked); }} /></td><td><label className="flex items-center gap-2"><input checked={sensitive} type="checkbox" onChange={(event) => { setSensitive(event.target.checked); }} />{sensitive ? <ShieldAlert className="size-4 text-amber-600" /> : null}</label></td><td><button className="btn-secondary" disabled={saving} onClick={() => { onSave(field.id, { display_name: name, semantic_type: semanticType, is_visible: visible, is_sensitive: sensitive }); }}>Guardar</button></td></tr>;
}

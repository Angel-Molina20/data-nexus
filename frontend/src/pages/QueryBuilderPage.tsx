...
import { QueryExecutionPanel } from "../features/query-execution/QueryExecutionPanel";
...
function Builder({ saved, auth, navigate, client }: { saved: Awaited<ReturnType<typeof getQuery>>; auth: ReturnType<typeof useAuth>; navigate: ReturnType<typeof useNavigate>; client: ReturnType<typeof useQueryClient> }) {
  ...
  return <div className="flex min-h-[calc(100vh-4rem)] flex-col bg-slate-100">
    ...
    <div className={`grid h-[calc(100vh-13rem)] min-h-[460px] flex-none overflow-hidden ${catalogOpen && inspectorOpen ? "lg:grid-cols-[280px_minmax(420px,1fr)_370px]" : catalogOpen ? "lg:grid-cols-[280px_1fr]" : inspectorOpen ? "lg:grid-cols-[1fr_370px]" : "grid-cols-1"}`}>{catalogOpen ? <div className="hidden min-h-0 overflow-hidden lg:block"><QueryCatalogPanel document={state.workingQuery} selectedSourceId={state.selectedSourceId} canUseSensitive={auth.hasPermission("queries.use_sensitive_fields")} onEntity={(sourceId) => { dispatch({ type: "select_source", sourceId }); }} onInspect={(entityId) => { const source = [state.workingQuery.query.source, ...state.workingQuery.query.joins.map((join) => join.source)].find((item) => item.entity_id === entityId); if (source) dispatch({ type: "select_source", sourceId: source.source_id }); }} onField={(fieldId, label) => { modify(queryActions.addField(state.workingQuery, state.selectedSourceId, fieldId, label)); }} /></div> : null}<main className="min-h-0 overflow-hidden"><QueryCanvas document={state.workingQuery} entities={entities} onLayout={layout} /></main>{inspectorOpen ? <div className="hidden min-h-0 overflow-hidden lg:block"><QueryInspectorPanel document={state.workingQuery} tab={state.selectedTab} readOnly={readOnly} onTab={(tab) => { dispatch({ type: "select_tab", tab }); }} onChange={modify} /></div> : null}</div>
    <QueryBottomPanel state={state} localProblems={problems} onTab={(tab) => { dispatch({ type: "bottom_tab", tab }); }} />
    <QueryExecutionPanel document={state.workingQuery} queryId={saved.id} revision={state.revision} canExecute={auth.hasPermission("queries.execute")} blocked={problems.length > 0 || Boolean(state.validation && !state.validation.valid)} />

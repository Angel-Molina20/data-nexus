...
function Choice({
  ...
  const fieldId = id ?? `choice-${generated.replaceAll(":", "")}`;
  const helpId = description ? `${fieldId}-help` : undefined;
  return (
    <div>
      <label
        className={cx(
          "flex min-h-10 items-start gap-3 text-sm text-foreground-secondary",
      ...
      >
        ...
        <span>
          ...
          {description ? (
            <span className="mt-0.5 block text-xs text-muted" id={helpId}>
              {description}
            </span>
          ) : null}
        </span>
